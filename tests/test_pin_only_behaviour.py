...
"""Behaviour tests for the pin-only check, ported from rsync-crypt.

rsync-crypt and docker-torrent-box-with-vpn were the only two of the six
repositories that tested this code at all. When rsync-crypt adopted the shared
library its copy of these 57 tests would otherwise have been deleted with the
script, so they moved here instead and now cover every consumer rather than
one.

They drive the real command line over stdin, which is the interface the gate
uses, and they build a real git repository per case so the check reads a base
file the way it does for an actual pull request. Graded against
examples/rsync-crypt.yml, the configuration that reproduces the script these
...
CONFIG = ROOT / "examples" / "rsync-crypt.yml"
SCRIPT = ROOT / "src" / "pin_only.py"
# The annotated_arg grammar reads its eligible ARG names from the Dockerfile
# this configuration names, so the checks that do not build their own
# repository are rooted at the fetched fixture that holds rsync-crypt's.
FIXTURE_ROOT = ROOT / "tests" / "originals" / "rsync-crypt"
...
def _check_in_repo(tmp_path, before: str, after: str, *, base: str | None = None):
    ...
    # The Dockerfile is what the annotated_arg grammar reads its eligible ARG
    # names from, so the throwaway repository needs the one this configuration
    # describes.
    (tmp_path / "Dockerfile").write_bytes(
        (ROOT / "tests" / "originals" / "rsync-crypt" / "Dockerfile").read_bytes()
    )
