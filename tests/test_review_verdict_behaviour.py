...
"""Behaviour tests for the review verdict, ported from rsync-crypt.

Same reason as tests/test_pin_only_behaviour.py: these 22 tests would have been
deleted along with rsync-crypt's copy of the script when it adopted the shared
library, and they are the only detailed coverage the verdict logic had. They
drive the real command line over stdin, which is the interface the gate uses.

The default bot list is rsync-crypt's, which is every repository's but
ivan-pinatti-labs/.github's; that one passes --no-bots and is covered by the
...
SCRIPT = ROOT / "src" / "review_verdict.py"
...
def test_bot_pull_request_whose_diff_is_not_pin_only_is_graded_like_a_human():
    # This is the injection case docker-torrent-box-with-vpn's #114 named
    # directly: a bot pull request that reaches outside its lane already gets
    # no automatic approval, so a person is already involved, and requiring a
    # real review here cannot stall the unattended path since there was
    # never an unattended path for this diff.
    result = _run(
        {
            "is_draft": False,
            "author": "renovate[bot]",
...
def test_a_forked_bot_login_does_not_reach_the_unattended_lane():
    ...
    # docker-torrent-box-with-vpn's #114 happened.
    result = _run(
        {
            "is_draft": False,
            "author": "renovate[bot]",
